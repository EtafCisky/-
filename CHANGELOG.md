...
## [3.5.24] - 2026-03-29 (Bug修复与功能优化)

### 🐛 Bug修复

**修复赠予命令无法排除bot QQ号的问题**

- 修复在群聊中使用"@bot 赠予 @用户 物品"时，会错误识别bot为目标用户的问题
- 解决方案：改用与决斗/切磋相同的逻辑，只识别命令关键词后面的At组件
- 新逻辑：遍历消息链，找到包含"赠予"的文本组件后，才开始识别At组件
- 效果：自动排除命令前的所有At（包括触发命令时艾特bot的情况）

### ✨ 功能优化

**完善丹药效果描述**

- 优化丹药效果描述，现在会显示所有正面和负面效果
- 显示规则：
  - **修为加成**：显示具体数值（如"增加1000修为"）
  - **突破率加成**：显示具体百分比（如"提升10%突破率"）
  - **其他效果**：只描述不显示数值
    - 气血：显示"恢复气血"或"损失气血"
    - 气血上限：显示"提升气血上限"或"降低气血上限"
    - 精神力：显示"提升精神力"或"损失精神力"
    - 攻击力：显示"提升攻击"或"降低攻击"
    - 防御力：显示"提升防御"或"降低防御"
    - 灵力：显示"提升灵力"或"损失灵力"
    - 灵石消耗：显示"消耗灵石"
...
- 赠予命令的At组件识别逻辑与决斗、切磋、管理员命令保持一致
- 丹药效果描述生成逻辑统一在 `alchemy_service.py` 和 `shop_service.py` 中
- 清空灵田洞天命令直接操作仓储层，确保数据完全删除
